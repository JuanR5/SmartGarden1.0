...
class DetectorA(Node):
    ...
    def listener_callbackB(self, data):
        ...
        # model
        model = YOLO("yolo-Weights/bestweigts.pt")
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                cv2.rectangle(img_rts, (x1, y1), (x2, y2), (255, 0, 255), 3)
                # print coordinates
                ...
                cv2.putText(img_rts, classNames[cls], org, font, fontScale, color, thickness)
        
            # Store the DETECTED frame
        #publiframe = self.br.cv2_to_imgmsg(img_rts, encoding='rgb8')


        self.filtered_frame = img_rts
